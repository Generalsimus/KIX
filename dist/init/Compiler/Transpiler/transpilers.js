...
exports.transpilerBefore = {
    ...
    [typescript_1.SyntaxKind.SourceFile]: (NODE, visitor, CTX) => {
        ...
        const ModuleColection = CTX.ModuleColection = utils_2.configModules(NODE, compilerOptions);
        if (NODE.imports.length) {
            const { externalImports } = typescript_1.collectExternalModuleInfo(CTX, NODE, CTX.getEmitResolver(), compilerOptions);
            externalImports.length && NODE.statements.splice(0, 0, CREATE_Const_Variable(externalImports.map((importNode) => {
                ...
                return [
                    typescript_1.getLocalNameForExternalImport(typescript_1.factory, importNode, NODE),
                    CREATE_Property_Access_Expression([compilerOptions.__Import_Module_Name, utils_1.getColumnName(Module_INDEX)])
                ];
            })));
        }
