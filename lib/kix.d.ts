...
type HTMLNodes = Text | HTMLElement | SVGElement;
type NodeAttributeMethodsType = {
  getAttr(a: string): ReturnType<Element["getAttribute"]>;
  setAttr(attribute: string, value: any): HTMLElement;
  Select(query: string): ReturnType<Element["querySelector"]>;
  Select(query: string[]): ReturnType<Element["querySelectorAll"]>;
  Child(index: number): HTMLNodes;
  Child(index: undefined): Element["children"];
  Inner(innerHTML: string): Text;
  Inner(innerHTML: undefined): HTMLElement["innerHTML"];
  setStyle(styleAttr: string | object): string;
  e(eventsObject: Record<string, (e: any) => void>): any;
  Parent(parentIndex?: number): ParentNode | null;
  NextElement(nextElementIndex?: number): HTMLElement | SVGElement | null;
  PreviousElement(
    previousElementIndex?: number
  ): HTMLElement | SVGElement | null;
  Append(childNode: any): HTMLNodes;
  Remove(): HTMLNodes;
  Replace(replaceNode: any): HTMLNodes;
  insert(method: "before" | "after", node: any): HTMLNodes;
  insert(method: any, node: any): undefined;
  Restart(): HTMLNodes;
};
/** Node is an interface from which a number of DOM API object types inherit. It allows those types to be treated similarly; for example, inheriting the same set of methods, or being tested in the same way. */
interface Node extends EventTarget, NodeAttributeMethodsType {
