...
export const NodeMethods = {
  ...
  routerBlock(routeObjectNode) {
    let tagName = routeObjectNode.tagName || "div",
      ...
      routerBlock = routeObjectNode.routerBlock,
      fakeNode = kix(null, ""),
      existNode = fakeNode,
      newRouteObjectNode = {
  ...
  router(routerObjectNode) {
    const to = flatFunction(routerObjectNode.path),
      unic = flatFunction(routerObjectNode.unic),
      component = flatFunction(routerObjectNode.component),
      fakeNode = kix(null, ""),
      escapeRegexp = [/[\-{}\[\]+?.,\\\^$|#\s]/g, "\\$&"],
      toPath = (unic ? [
        escapeRegexp,
      ...
      routeRegExp = new RegExp(unic ? toPath : "^" + toPath + "$", "i"),
      nodes = {},
      createRouterNode = (ID) => {
        var loc_path = decodeURI(document.location.pathname),
          mathch_path = loc_path.match(routeRegExp) || [];
...
export const AttributeMethods = {
  ...
  setAttr(attribute, value) {
    ...
    value instanceof Function ?
      ...
      (AttributeMethods[attribute] ? this[attribute](value, attribute, this) : this.setAttribute(attribute, value));
    return this;
  },
  Select(query) {
  ...
  Insert(method, node) {
    let parent = this.Parent(),
      HtmlNode = kix(null, flatFunction(node, parent, objectPropertyRegistrator));
    switch (method) {
      case "after":
  ...
  Restart() {
    let node = kix(null,
      flatFunction(this.KNode, this.Parent(), objectPropertyRegistrator)
    );
    this.Replace(node);
    return node;
  },
...
export function createNodeApp(createElement) {
  const createKixElement = (objectNode, createdNode) => {
    for (var objectNodeProperty in objectNode) {
      if (createdNode) {
        ...
        if (NodeMethods[objectNodeProperty]) {
          return NodeMethods[objectNodeProperty](objectNode, objectNodeProperty, createElement, createKixElement);
        } else {
